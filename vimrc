...
" PageDown calls the '.runFromVim.sh' for the current line only. 
...
" Why PageDown? that's what the remote control sends on Windows. For Mac, you can play with Karabiner-Elements, depending on the device (I have a Logitech pointer).
...
" F10 run the selected lines only
map       <F10> :call system('tmux set-option status-right ' . expand('%:t') . ':' . line('.') . shellescape(' %H:%M') )<CR>z.:'<,'>w !sh ~/.runFromVim.sh F10

" PageUp sets a tmux buffer with the selected lines and send it directly to the console with paste-buffer. (no execution)
